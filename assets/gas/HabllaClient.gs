// GENERATED by tooling/build-gas.js from the hablla SDK. Do not edit.
      ...
      return __async(this, null, function* () {
        for (let attempt = 0; ; attempt++) {
          ...
          try {
            ...
            const backoff = Math.min(500 * __pow(2, attempt), 8e3) + Date.now() % 250;
        ...
        for (let attemptNum = 0; ; attemptNum++) {
          try {
            ...
            if (transient && attemptNum < 6) {
              yield sleep2(Math.min(1e3 * Math.pow(2, attemptNum), 2e4) + Date.now() % 400);
              continue;
            }
            throw err;
          }
  ...
  var HabllaClient = class {
    constructor(config) {
      ...
      this.auth = new HabllaAuth(
        config.transport,
        { workspaceToken: (_b = config.workspaceToken) != null ? _b : "", refreshToken: config.refreshToken, firebaseApiKey: config.firebaseApiKey, baseUrl },
