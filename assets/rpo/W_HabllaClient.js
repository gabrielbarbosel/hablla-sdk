...
class W_HabllaClient {
    constructor() {
        this.name = "W_HabllaClient";
    }
    async execute() {
        const g = typeof globalThis !== "undefined" ? globalThis : global;
        if (g.__W_HABLLACLIENT_INSTALLED__) return { ok: true };
        g.__W_HABLLACLIENT_INSTALLED__ = true;
"use strict";
(() => {
  ...
  var HabllaAuth = class {
    constructor(transport, config, cache = new MemoryStrategyCache()) {
      ...
      this.maxBackoffMs = config.retry?.maxBackoffMs ?? 8e3;
    }
    transport;
    config;
    cache;
    ...
    async resolveStrategy(cacheKey) {
      if (!this.loaded) {
    ...
    async recordStrategy(cacheKey, strategy) {
    ...
    async refreshOnce() {
      ...
      const expiresAt = Date.now() + Number(res.data.expires_in ?? 3600) * 1e3;
  ...
  var HabllaHttpClient = class {
    constructor(transport, auth, config) {
      ...
      this.maxAttempts = config.retry?.maxAttempts ?? 6;
      this.maxBackoffMs = config.retry?.maxBackoffMs ?? 2e4;
    }
    transport;
    auth;
    config;
     ...
     */
    async request(method, rawPath, opts = {}) {
      const sleep2 = (ms) => new Promise((r) => setTimeout(r, ms));
      const idempotent = method === "GET" || method === "HEAD";
      for (let attemptNum = 0; ; attemptNum++) {
        try {
          return await this._requestOnce(method, rawPath, opts);
        } catch (err) {
          ...
          if (transient && attemptNum < this.maxAttempts - 1) {
            await sleep2(Math.min(1e3 * Math.pow(2, attemptNum), this.maxBackoffMs) + Date.now() % 400);
            continue;
          }
          throw err;
        }
  ...
  var MassDispatch = class {
     ...
     */
    async run(contacts, spec, sleep2 = (ms) => new Promise((resolve) => setTimeout(resolve, ms))) {
      ...
      const audienceCount = await this.waitForAudience(segmentationId, imported, spec.indexTimeoutMs ?? DEFAULT_INDEX_TIMEOUT_MS, sleep2);
      let campaignId;
      let status = "sent";
      if (audienceCount > 0) {
     ...
     */
    async dispatchPersonalized(contacts, config, sleep2 = (ms) => new Promise((resolve) => setTimeout(resolve, ms))) {
      ...
      const result = await this.run(enriched, spec, sleep2);
     ...
     */
    async waitForAudience(segmentationId, expected, timeoutMs, sleep2) {
  ...
  // src/runtime/rpo/global-strategy-cache.ts
  var GlobalStrategyCache = class {
    bag() {
      return globalThis.HABLLA_CACHE;
    }
    async load() {
...
await new W_HabllaClient().execute();
