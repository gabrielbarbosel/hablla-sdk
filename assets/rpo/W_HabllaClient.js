// GENERATED by tooling/build-rpo.js from the hablla SDK. Do not edit.
    ...
    async refresh() {
      for (let attempt = 0; ; attempt++) {
        ...
        try {
          ...
          const backoff = Math.min(500 * 2 ** attempt, 8e3) + Date.now() % 250;
      ...
      for (let attemptNum = 0; ; attemptNum++) {
        try {
          ...
          if (transient && attemptNum < 6) {
            await sleep2(Math.min(1e3 * Math.pow(2, attemptNum), 2e4) + Date.now() % 400);
            continue;
          }
          throw err;
        }
