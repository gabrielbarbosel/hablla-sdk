...
/**
 * A user as returned by the workspace member endpoints (e.g. `workspaces.getUsers`,
 * `sectors.getSectorsUsers`, `sectors.listUsers`). This is the real shape of a
 * workspace user — distinct from {@link Workspace} — and is what consumers use to
 * resolve advisors/attendants. Hand-maintained here because the generated
 * `users`/`wsusers` resources carry no entity interface; the codegen template
 * should be updated to emit this and reuse it for those user-returning methods.
 */
